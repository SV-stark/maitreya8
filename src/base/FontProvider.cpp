...
FontProvider::FontProvider() {
  ...
  // Modernize: Set base size to 11 for better readability
  defaultfd[FONT_GRAPHIC_DEFAULT].pointSize = 11;

#if defined(__WXMSW__)
  // Modern Windows Font
  defaultfd[FONT_GRAPHIC_DEFAULT].facename = wxT("Segoe UI");
  defaultfd[FONT_PLAIN_DEFAULT].facename = wxT("Consolas");
#elif defined(__WXMAC__)
  defaultfd[FONT_GRAPHIC_DEFAULT].facename = wxT("Helvetica Neue");
  defaultfd[FONT_PLAIN_DEFAULT].facename = wxT("Menlo");
#endif

  defaultfd[FONT_TEXT_HEADER].weight = wxFONTWEIGHT_BOLD;
  defaultfd[FONT_TEXT_HEADER].pointSize = 14; // Increased from 12
...
wxFont *FontProvider::getFallbackFont(const FONT_ID &id) {
  ...
  return wxTheFontList->FindOrCreateFont(
      defaultfd[id].pointSize, defaultfd[id].family, defaultfd[id].style,
      defaultfd[id].weight, defaultfd[id].underline, defaultfd[id].facename);
}
...
***
***************************************************************/
wxString FontProvider::getSymbolFontName() { return SYMBOL_FONT_NAME; }
...
wxFont *FontProvider::getFontZoom(const FONT_ID &id, const double &zoom) {
  ...
  if (!config->font[id].isOk()) {
    printf(
        "FontProvider::getFont setting default font description for font #%d\n",
        id);
    config->font[id] = defaultfd[id];
  }

  int size = zoom == 1.0 ? config->font[id].pointSize
                         : (int)(zoom * (double)config->font[id].pointSize);
  // printf( "Font Size is %d, zoom was %f\n", size, zoom );
  if (size <= 0 || size > MAX_FONT_SIZE) {
    printf("ERROR FontProvider::getFontZoom; invalid font size %d for id %d, "
           "zoom was %f, setting default\n",
           size, (int)id, zoom);
    size = 10;
  }

  /*
  wxFont *font = wxTheFontList->FindOrCreateFont( size, config->font[id].family,
  config->font[id].style, config->font[id].weight, config->font[id].underline,
  config->font[id].facename ); printf( "FontProvider::getFontZoom mark 1 %ld
  millisec\n", (wxGetLocalTimeMillis() - starttime).ToLong()); return font;
  */

  return wxTheFontList->FindOrCreateFont(
      size, config->font[id].family, config->font[id].style,
      config->font[id].weight, config->font[id].underline,
      config->font[id].facename);
}
...
wxFont *FontProvider::getFontBySize(const FONT_ID &id, const int &pointSize) {
  ...
  if (pointSize <= 0 || pointSize > MAX_FONT_SIZE) {
    printf("ERROR FontProvider::getFont; invalid font size %d for id %d, "
           "setting default\n",
           pointSize, (int)id);
    return getFont(id);
  }
  ...
  // fd[id].toString()));
  return wxTheFontList->FindOrCreateFont(
      pointSize, config->font[id].family, config->font[id].style,
      config->font[id].weight, config->font[id].underline,
      config->font[id].facename);
}
...
wxFont *FontProvider::getFont(const FONT_ID &id, const int increase) {
  ...
  if (size <= 0 || size > MAX_FONT_SIZE) {
    printf("ERROR FontProvider::getFont; invalid font size %d for id %d, "
           "increase was %d, setting default\n",
  ...
  // fd[id].toString()));
  return wxTheFontList->FindOrCreateFont(
      config->font[id].pointSize + increase, config->font[id].family,
      config->font[id].style, config->font[id].weight,
      config->font[id].underline, config->font[id].facename);
}
