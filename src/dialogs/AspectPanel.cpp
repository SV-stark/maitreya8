...
const wxEventType COMMAND_SELECTED_ASPECT_CHANGED = wxID_HIGHEST + 9002;
const wxEventType COMMAND_ASPECT_DOUBLECLICK = wxID_HIGHEST + 9003;

enum {
  CD_PREVIEW_PANEL = wxID_HIGHEST + 1,
  CD_STYLE_CHOICE,
  CD_CHECK_ACTIVE,
  CD_BUTTON_PEN,
  CD_SPIN_ORBIS
...
public:
  ...
  **
  ******************************************************/
  AspectPreviewPanel(wxWindow *parent, wxWindowID id,
                     std::vector<AspectConfig> &a)
      : wxPanel(parent, id, wxDefaultPosition, wxDefaultSize,
                wxTAB_TRAVERSAL | wxBORDER_SUNKEN | wxFULL_REPAINT_ON_RESIZE),
        configs(a) {
    showSymbols = true;
    selected = 0;
    ...
    Connect(wxEVT_LEFT_DOWN,
            wxMouseEventHandler(AspectPreviewPanel::OnMouseLeftDown));
    Connect(wxEVT_LEFT_DCLICK,
            wxMouseEventHandler(AspectPreviewPanel::OnMouseLeftDoubleClick));
  }
...
private:
  ...
  **
  ******************************************************/
  void drawSingleAspect(Painter *painter, const int &aspectindex,
                        const int &xleft, const int &xright, const int &y) {
    wxChar c;
    const int xh = 15;
    ...
    if (showSymbols) {
      ...
      if (!c) {
        ...
        painter->drawTextFormatted(
            MRect(xleft + w / 2 - xh, y - xh, 2 * xh, 2 * xh),
            AspectExpert::getAspectShortDescription(aspectindex),
            Align::Center);
      } else {
        painter->setSymbolFont();
        painter->drawTextFormatted(
            MRect(xleft + w / 2 - xh, y - xh, 2 * xh, 2 * xh), c,
            Align::Center);
      }
    } else
  ...
  ******************************************************/
  void OnPaint(wxPaintEvent &) {
    ...
    painter.drawRectangle(selborder + col2 * w / 2, selborder + sely * steplen,
                          w / 2 - 2 * selborder, steplen - 2 * selborder);

    for (int i = 0; i < imed; i++) {
      drawSingleAspect(&painter, i, xborder, w / 2 - xborder,
                       steplen / 2 + i * steplen);
    }
    for (int i = imed; i < MAX_ASPECT_PANEL_ITEMS; i++) {
      drawSingleAspect(&painter, i, w / 2 + xborder, w - xborder,
                       steplen / 2 + (i - imed) * steplen);
    }
  }
  ...
  ***************************************************************/
  int getSelectedItemByMousePos(wxMouseEvent &event) {
    ...
    if (sel >= MAX_ASPECT_PANEL_ITEMS)
      sel = MAX_ASPECT_PANEL_ITEMS - 1;
    if (sel < 0)
      sel = 0;
    return sel;
  }
...
AspectPanel::AspectPanel(wxWindow *parent) : ConfigPanel(parent) {
  ...
  const wxString choice_aspect_type_choices[] = {
      _("dummy"), _("dummy"), _("dummy"), _("dummy"), _("dummy"),
  };
  choice_aspect_type =
      new wxChoice(this, CD_STYLE_CHOICE, wxDefaultPosition, wxDefaultSize, 5,
                   choice_aspect_type_choices);
  ...
  spin_orbis = new wxSpinCtrl(this, CD_SPIN_ORBIS, wxT(""), wxDefaultPosition,
                              wxDefaultSize, wxSP_ARROW_KEYS, 1, 20);
  ...
  for (int i = 0; i < MAX_ASPECT_PANEL_ITEMS; i++)
    choice_aspect_type->Append(AspectExpert::getAspectName(i));
  selconfig = 0;
  choice_aspect_type->SetSelection(selconfig);

  preview_panel->Refresh();

  Connect(CD_STYLE_CHOICE, wxEVT_COMMAND_CHOICE_SELECTED,
          wxCommandEventHandler(AspectPanel::OnAspectChoice));
  Connect(CD_CHECK_ACTIVE, wxEVT_COMMAND_CHECKBOX_CLICKED,
          wxCommandEventHandler(AspectPanel::OnCheckActive));
  Connect(CD_SPIN_ORBIS, wxEVT_COMMAND_SPINCTRL_UPDATED,
          wxSpinEventHandler(AspectPanel::OnSpinOrbis));
  Connect(CD_BUTTON_PEN, wxEVT_COMMAND_BUTTON_CLICKED,
          wxCommandEventHandler(AspectPanel::OnButtonPen));
  Connect(CD_PREVIEW_PANEL, COMMAND_SELECTED_ASPECT_CHANGED,
          wxCommandEventHandler(AspectPanel::OnSelectedAspectChanged));
  Connect(CD_PREVIEW_PANEL, COMMAND_ASPECT_DOUBLECLICK,
          wxCommandEventHandler(AspectPanel::OnAspectDoubleClick));
}
...
void AspectPanel::do_layout() {
  ...
  wxStaticBoxSizer *sizer_preview = new wxStaticBoxSizer(
      new wxStaticBox(this, wxID_ANY, _("Preview and Aspect Selection")),
      wxHORIZONTAL);
  wxFlexGridSizer *sizer_left = new wxFlexGridSizer(2, 1, 0, 0);
  wxStaticBoxSizer *sizer_options = new wxStaticBoxSizer(
      new wxStaticBox(this, wxID_ANY, _("Options")), wxVERTICAL);
  wxGridSizer *grid_options = new wxGridSizer(2, 2, 0, 0);
  wxStaticBoxSizer *sizer_type = new wxStaticBoxSizer(
      new wxStaticBox(this, wxID_ANY, _("Aspect Type")), wxVERTICAL);
