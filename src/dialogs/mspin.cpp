...
/*************************************************/ /**
                                                     *
                                                     *  delegates transfer* and
                                                     * validate stuff to text
                                                     * control
                                                     *
                                                     ******************************************************/
...
public:
  ...
  DelegatingSpinValidator(const DelegatingSpinValidator &tocopy)
      : wxValidator(), ctrl(tocopy.ctrl) {}
...
public:
  MSpinCtrlTextGeneric(MBaseSpin *spin, const wxWindowID id = -1)
      : wxTextCtrl(spin, id, wxEmptyString, wxDefaultPosition, wxDefaultSize,
                   0) {
    m_spin = spin;
...
public:
  MSpinCtrlButtonGeneric(MBaseSpin *spin, wxWindowID id)
      : wxSpinButton(spin, id, wxDefaultPosition, SPIN_SIZE,
...
**
******************************************************/
MBaseSpin::MBaseSpin(wxWindow *parent, int id, const wxPoint pos,
                     const wxSize size, const bool showLabel)
    : wxControl(parent, id, pos, size, wxBORDER_NONE | wxTAB_TRAVERSAL),
      value(0), maxvalue(0) {
  SetExtraStyle(wxWS_EX_VALIDATE_RECURSIVELY);

  // spin = new wxSpinButton( this, BDT_SPIN, wxDefaultPosition, wxDefaultSize,
  // wxSP_ARROW_KEYS | wxSP_WRAP );
  ...
  if (showLabel) {
    label = new wxStaticText(this, BDT_LABEL, wxT("--"), wxDefaultPosition,
                             wxSize(30, -1));
...
int MBaseSpin::findToken4add() {
  ...
  // s ), maxvalue, text->GetInsertionPoint());
...
**
******************************************************/
MDateSpin::MDateSpin(wxWindow *parent, int id, const wxPoint pos,
                     const wxSize size)
...
void MDateSpin::updateLabel() {
  ...
  Lang lang;
  assert(label);
  const double tmpjd = *value + (config->defaultLocation->getTimeZone() +
                                 config->defaultLocation->getDST()) /
                                    24.0;
