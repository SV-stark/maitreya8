...
/*************************************************/ /**
                                                     *
                                                     * \brief abstract class for
                                                     * paiting graphical items
                                                     * and text on a GUI
                                                     *
                                                     ******************************************************/
class Painter {
public:
  Painter(WriterConfig * = (WriterConfig *)NULL,
  ...
  // drawing methods
  virtual void drawRectangle(const double &x, const double &y, const double &w,
                             const double &h, const double = 0) = 0;
  void drawRectangle(const MRect &, const double = 0);
  virtual void drawPolygon(int n, MPoint points[], wxCoord xoffset = 0,
                           wxCoord yoffset = 0) = 0;
  virtual void drawLine(const double &x1, const double &y1, const double &x2,
                        const double &y2) = 0;
  void drawLine(const MPoint &p1, const MPoint &p2);
  virtual void drawEllipse(const double &x0, const double &y0,
                           const double &xmax, const double &ymax) = 0;
  virtual void drawArc(const double &x0, const double &y0, const double &xmax,
                       const double &ymax, const double &w1,
                       const double &w2) = 0;
  virtual void drawPoint(const double &x, const double &y) = 0;

  virtual void drawBitmap(const wxBitmap &, const double &, const double &,
                          const bool &) = 0;
  ...
  virtual void drawLinearGradient(const MRect &r, const wxColour &startColor,
                                  ...
                                  const int &direction) {
  } // Default empty implementation or pure virtual? Better pure or default.
    // Default avoids breaking other subclasses if any. But DcPainter needs it.
  // Actually pure virtual is better if we want to enforce it, but let's make it
  // virtual with empty body to be safe for PdfPainter etc. for now. Check
  // PdfPainter later.
  ...
  virtual void drawRotatedText(wxString /*text*/, const double & /*x*/,
                               const double & /*y*/,
...
/*************************************************/ /**
                                                     *
                                                     * \brief Painter
                                                     * implementation for wx
                                                     * device context (GUI and
                                                     * file export via memory
                                                     * DC)
                                                     *
                                                     ******************************************************/
class DcPainter : public Painter {
public:
  DcPainter(wxDC *, WriterConfig * = (WriterConfig *)NULL,
            ColorConfig * = (ColorConfig *)NULL);
  ...
  virtual void drawArc(const double &x0, const double &y0, const double &xmax,
                       const double &ymax, const double &w1, const double &w2);
  ...
  virtual void drawRotatedText(wxString s, const double &x, const double &y,
                               const double &angle);
