...
public:
  SimpleHelpPopupWidget(wxWindow *parent, wxString title, wxString message)
    ...
    wxStaticText *text1 =
        new wxStaticText(panel, wxID_ANY, title, wxDefaultPosition,
...
void showHelpPopup(wxWindow *parent, wxString header, wxString contents) {
  SimpleHelpPopupWidget *popup =
      new SimpleHelpPopupWidget(parent, header, contents);
...
int doMessageBox(wxWindow *parent, const wxString message, const long style) {
  return wxMessageDialog(parent, message, GUI_APP_NAME, style | wxCENTRE)
      .ShowModal();
}
...
/*************************************************/ /**
                                                     ...
                                                     *
                                                     ******************************************************/
DoNotShowAgainDialog::DoNotShowAgainDialog(wxWindow *parent, wxString message,
                                           bool *item, const int style)
    : wxDialog(parent, -1, _("Information"), wxDefaultPosition, wxDefaultSize,
  ...
  check_not_ask_again =
      new wxCheckBox(this, wxID_ANY, _("Do not show this message again"));
  // check_not_ask_again->SetValue( *cfgitem );

  if (style & wxOK) {
    okbutton = new wxButton(this, wxID_OK, _("OK"));
    sizer_button->Add(okbutton, 0, wxALL | wxALIGN_CENTER, 8);
  }
  if (style & wxCANCEL) {
    cancelbutton = new wxButton(this, wxID_CANCEL, _("Cancel"));
    sizer_button->Add(cancelbutton, 0, wxALL | wxALIGN_CENTER, 8);
  }
  if (style & wxYES_NO) {
    yesbutton = new wxButton(this, wxID_YES, _("Yes"));
    sizer_button->Add(yesbutton, 0, wxALL | wxALIGN_CENTER, 8);
    nobutton = new wxButton(this, wxID_NO, _("No"));
    sizer_button->Add(nobutton, 0, wxALL | wxALIGN_CENTER, 8);
  }

  wxStaticBitmap *bmp = new wxStaticBitmap(
      this, -1, ImageProvider::get()->getBitmap(BM_MAITREYA));
  sizer_lr->Add(bmp, 0, wxALL | wxALIGN_CENTER_VERTICAL, 30);

  sizer_text->Add(messageLabel, 0, wxALL, 12);
  sizer_text->Add(check_not_ask_again, 0, wxALL, 12);
  ...
  Connect(wxID_ANY, wxEVT_COMMAND_BUTTON_CLICKED,
          wxCommandEventHandler(DoNotShowAgainDialog::OnButton));
}
...
void DoNotShowAgainDialog::OnButton(wxCommandEvent &event) {
  ...
  if (check_not_ask_again->IsChecked())
    *cfgitem = false;
