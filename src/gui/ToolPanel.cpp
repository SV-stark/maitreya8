...
const int tb_eclipsewindow[] = {TBS_YEAR, TBS_NBYEARS, TBS_ECL_TYPE, TBS_TZ,
                                -1};

const int tb_printpreviewwindow[] = {TBS_PRINTOUTTYPE,    TBS_SHEETSTYLE,
                                     TBS_VEDICCHARTSTYLE, TBS_WESTERNCHARTSTYLE,
                                     CMD_LAUNCH_VIEWER,   -1};

const int tb_transitwindow[] = {
    TBS_DATE,          TBS_TIME,     CMD_NOW,         TBS_TZ,
    TBS_DATE_PROGRESS, TB_SEPARATOR, TBS_TRANSITMODE, -1};

const int tb_partnerwindow[] = {TBS_PARTNER1, TB_SEPARATOR, TBS_PARTNER2, -1};

const int tb_uranianwindow[] = {
    TBS_SORT, CMD_FILTER,    TBS_ORBIS, TBS_GRADKREIS,
    TBS_YEAR, APP_CONFIGURE, -1};

const int tb_uranianchartwindow[] = {TBS_URANIAN_CHART_REFOBJECT, TBS_ORBIS,
                                     -1};

const int tb_gdasawindow[] = {
    CMD_GOTO_YEAR,      CMD_NOW,       CMD_SELECT_DASA,
    TB_SEPARATOR,       TBC_DASA_ZOOM, CMD_ZOOM_ORIGINAL,
    CMD_ZOOM_IN,        CMD_ZOOM_OUT,  TB_SEPARATOR,
    TBS_ANTARDASALEVEL, TB_TEXT,       -1};

const int tb_ephemwindow[] = {TBS_YEAR,     TBS_MONTH,     CMD_NOW,
                              TB_SEPARATOR, TBS_EPHEMMODE, TBS_TZ,
                              TBS_DASA,     TBS_EPHEMDEG,  -1};
const int tb_solarwindow[] = {TBS_YEAR, CMD_NOW, CMD_NEW_ANNUAL_CHART, TB_TEXT,
                              -1};

const int tb_dasawindow[] = {CMD_ENTRY_CHART, CMD_TOGGLE_SPLIT, TB_SEPARATOR,
                             TBS_YOGASOURCEFILTER, -1};

const int tb_yogawindow[] = {TBS_VARGA, TBS_YOGAGROUPFILTER,
                             TBS_YOGASOURCEFILTER, TBS_YOGAFIRSTHOUSE, -1};

const int tb_yogaeditor[] = {APP_NEW,    APP_OPEN,  CMD_SAVE,
                             CMD_SAVEAS, CMD_CLOSE, -1};
...
**
******************************************************/
ToolbarFactory::ToolbarFactory() { init(); }
...
wxToolBar *ToolbarFactory::createToolbar(wxWindow *parent, const int &type) {
  ...
  if (a)
    ...
    wxLogWarning(wxT("Warning: no toolbar configuration for id #%d found"),
                 type);
  toolbar->Realize();
  return toolbar;
}
...
void ToolbarFactory::create(wxToolBar *toolbar, const int *a) {
  ...
  while (*i != -1) {
    ...
    if (IS_TB_CHOICE_ITEM(*i))
      createChoiceItem(toolbar, *i);
    else if (IS_TB_TEXT_ITEM(*i))
      createTextField(toolbar);
    else if (IS_TB_COMBO_ITEM(*i))
      createComboItem(toolbar, *i);
    else if (*i == TB_SEPARATOR)
      toolbar->AddSeparator();
    else
      createByCommandId(toolbar, *i);
...
void ToolbarFactory::createByCommandId(wxToolBar *toolbar, const int &item) {
  ...
  const wxItemKind kind =
      (item == CMD_TOGGLE_SPLIT ? wxITEM_CHECK : wxITEM_NORMAL);
...
void ToolbarFactory::createTextField(wxToolBar *toolbar) {
  ...
  control = (wxControl *)new ToolbarLabel(toolbar, TB_TEXT, wxDefaultPosition,
                                          wxSize(600, textHeight));
...
void ToolbarFactory::createComboItem(wxToolBar *toolbar, const int &item) {
  ...
  const static wxString zoom_choices[] = {
      wxT("10"),  wxT("25"),  wxT("50"),  wxT("100"), wxT("150"),  wxT("200"),
      wxT("250"), wxT("300"), wxT("400"), wxT("500"), wxT("1000"), wxT("3000")};
  wxComboBox *combo = 0;
  wxString label;
  ...
  if (item == TBC_DASA_ZOOM) {
    ...
    combo =
        new wxComboBox(toolbar, item, wxT("100"), wxDefaultPosition,
                       wxSize(120, -1), 12, zoom_choices, wxTE_PROCESS_ENTER);
...
void ToolbarFactory::createChoiceItem(wxToolBar *toolbar, const int &item) {
  ...
  const static wxString antardasa_choices[] = {
      wxT("Mahadasa"), wxT("Antardasa"), wxT("Pratyantardasa"),
      wxT("Sookshmantardasa"), wxT("Pranadasa")};

  const static wxString ephem_mode_choices[] = {
      _("Longitudes"), _("Graphical"), _("Sunrise/Tithi"),
      _("Ingress"),    _("Lunar"),     _("KP")};

  const static wxString tz_choices[] = {wxT("UT"), _("Local")};

  const static wxString ephemdeg_choices[] = {
      wxT("360"),   wxT("180"),   wxT("90"),    wxT("45"),     wxT("30"),
      wxT("22:30"), wxT("13:20"), wxT("11:15"), wxT("5:07:30")};

  const static wxString first_house_choices[] = {
      _("Ascendant"), _("Moon"),     _("Arudha Lagna"), _("Aries"),
      _("Taurus"),    _("Gemini"),   _("Cancer"),       _("Leo"),
      _("Virgo"),     _("Libra"),    _("Scorpio"),      _("Sagittarius"),
      _("Capricorn"), _("Aquarius"), _("Pisces")};
  ...
  case TBS_VARGA: {
    ...
    wxChoice *c =
        new wxChoice(toolbar, item, wxDefaultPosition, wxSize(150, -1));
    for (Varga i = V_FIRST; i <= V_LAST; i++) {
      if (loader->getVargaDivision(i) > 0)
        s.Printf(wxT("%s (D%d)"), loader->getVargaName(i).c_str(),
                 loader->getVargaDivision(i));
      else
        s = loader->getVargaName(i);
      c->Append(s);
    }
    label = _("Varga");
    control = c;
  ...
  case TBS_YEAR:
    control =
        new wxSpinCtrl(toolbar, item, wxT(""), wxDefaultPosition, wxDefaultSize,
  ...
  case TBS_SHEETSTYLE:
    ...
    break;
  case TBS_NBYEARS:
    control = new wxSpinCtrl(toolbar, item, wxT(""), wxDefaultPosition,
                             wxDefaultSize, wxSP_ARROW_KEYS, 1, 10);
    control->SetSize(60, control->GetBestSize().GetY());
    label = _("Years");
    break;
  case TBS_ECL_TYPE:
    control = new wxChoice(toolbar, item, wxDefaultPosition, wxDefaultSize, 3,
                           eclipse_types, 0);
    label = _("Type");
    break;
  case TBS_DATE_PROGRESS:
    control = new wxSlider(toolbar, item, 0, -7, 7, wxDefaultPosition,
  ...
  case TBS_DATE:
    ...
    // control = new MSpinDate( toolbar, item, wxDefaultPosition, wxSize( 150,
    // -1 ));
    label = _("Date");
    break;
  case TBS_TIME:
  ...
  case TBS_ORBIS:
    control = new wxSpinCtrl(toolbar, item, wxT(""), wxDefaultPosition,
                             wxDefaultSize, wxSP_ARROW_KEYS, 0, 300);
    control->SetSize(60, control->GetBestSize().GetY());
    label = _("Orbis");
    break;
  case TBS_GRADKREIS:
  ...
  case TBS_YOGAFIRSTHOUSE:
    ...
    // control = new wxChoice( toolbar, item, wxDefaultPosition, wxSize( 120, -1
    // ), 15, first_house_choices, wxADJUST_MINSIZE );
    control = new wxChoice(toolbar, item, wxDefaultPosition, wxSize(120, -1),
                           15, first_house_choices);
    label = _("Ascendant");
    break;
  case TBS_ANTARDASALEVEL: {
    wxChoice *c = new wxChoice(toolbar, item, wxDefaultPosition, wxDefaultSize,
                               5, antardasa_choices, 0);
    c->SetSelection(1);
    control = c;
  ...
  case TBS_MONTH:
    ...
    break;
  case TBS_EPHEMMODE:
    control = new wxChoice(toolbar, item, wxDefaultPosition, wxSize(120, -1), 6,
                           ephem_mode_choices, 0);
    label = _("Mode");
    break;
  case TBS_TZ:
    control = new wxChoice(toolbar, item, wxDefaultPosition, wxDefaultSize, 2,
                           tz_choices, 0);
    label = _("Time Zone");
    break;
  case TBS_EPHEMDEG:
    control = new wxChoice(toolbar, item, wxDefaultPosition, wxSize(120, -1), 9,
                           ephemdeg_choices, 0);
    label = _("Circle Type");
    break;
  case TBS_PARTNER1:
...
void ToolbarFactory::populateMainToolbar(wxToolBar *tb) {
  ...
  tb->SetToolBitmapSize(wxSize(ImageProvider::get()->getDefaultBitmapSize(),
                               ImageProvider::get()->getDefaultBitmapSize()));
...
**
******************************************************/
void ToolbarFactory::addTool(wxToolBar *tb, const int &id,
                             const wxItemKind kind) {
  ...
  if (!command) {
    ...
    tb->AddTool(id, command->getShortText(),
                ip->getBitmap(command->getBitmapId()), command->getText(),
                kind);
    /*
    if ( config->toolbar->toolbarIconSize )
    {
            // 32x32
            tb->AddTool( id, command->getShortText(), ip->getBitmap(
    command->getBitmapId() ), command->getText(), kind );
    }
    else if ( id )
    {
            // 24x24
            tb->AddTool( id, command->getShortText(), ip->getScaledBitmap(
    command->getBitmapId(), 24 ), command->getText(), kind );
    }
    */
  }
}
...
**
******************************************************/
void ToolbarFactory::addControl(wxToolBar *toolbar, wxControl *control,
