...
**
******************************************************/
WesternChart::WesternChart(const ChartType &charttype,
...
void WesternChart::setupAries() {
  ...
  if (wconf->forcedZodiacStart >= 0 && wconf->forcedZodiacStart < 360)
    aries = wconf->forcedZodiacStart;
}
...
**
******************************************************/
double WesternChart::getVisualObjectLongitude(const Horoscope *h,
                                              const ObjectId &p) {
  assert(h);
...
void WesternChart::paintInternal(ChartConfig *) {
  ...
  if (wconf->image.show == true && !wconf->image.filename.IsEmpty())
    drawInlayImage();

  if (h1) {
    // if ( chartprops->getWesternChartDisplayConfig().showHouses )
    // drawHouses();
  }
  if (wconf->eastIndianCenter.show)
    drawEastIndianChartCenter();
  if (wconf->signs.show)
    drawSigns();
  ...
  if (h1) {
    if (chartprops->getWesternChartDisplayConfig().showAspects)
      drawAspects();
    if (chartprops->getWesternChartDisplayConfig().showHouses)
      drawHouses();
    drawAllPlanets(h1);
    ...
    if (h2) {
      WesternChartConfig *wconf = getWChartConfig();
      const int tstyle =
          chartprops->getWesternChartDisplayConfig().secondchartStyle;
      paintObjectRing(aries, tstyle > 0 ? wconf->rInnerTransitPlanets
                                        : wconf->rOuterTransitPlanets);
...
**
******************************************************/
double WesternChart::getLongitude4FrameSegment(const int &i,
                                               const bool alignToHouses) {
  int i1 = i % 12;
  if (alignToHouses) {
...
void WesternChart::drawPalettes() {
  ...
  for (std::list<GcChartFrameBgPattern>::iterator iter =
           wconf->patterns.begin();
       iter != wconf->patterns.end(); iter++) {
    ...
    if (palette) {
      ...
      wxLogError(wxT("palette '%s' in chart '%s' not available"),
                 iter->paletteName.c_str(), wconf->name.c_str());
    }
  }
...
**
******************************************************/
void WesternChart::drawSingleAspect(const double &len1, const double &len2,
                                    const ASPECT_TYPE &type) {
  double glen1, glen2;
  wxString s;
  ...
  if (chartprops->getWesternChartDisplayConfig().showAspectSymbols &&
      type != 0) {
    ...
    if (s.IsEmpty()) {
      ...
      painter->drawTextFormatted(
          rect, AspectExpert::getAspectShortDescription(type), Align::Center);
    } else {
      setSymbolFont();
...
void WesternChart::drawAspects() {
  ...
  painter->setPen(wxPen(*wxRED, 1, wxPENSTYLE_DOT_DASH));
  ...
  {
          paintLengthMarker( aries + haspecting->getWesternLongitude(
  (*iter).planet1 ), wconf->aspects.radius ); paintLengthMarker( aries +
  haspected->getWesternLongitude( (*iter).planet2 ), wconf->aspects.radius );
  }
  */
  ...
  for (iter = al.begin(); iter != al.end(); iter++) {
    ...
    // (int)h2marked, (int)iter->planet1, (int)iter->planet2 );

    zoom = smallzoom;
    if (chartprops->getWesternChartBehaviorConfig().magnifyAspects) {
      switch (charttype) {
      case CT_RADIX:
        if (p1marked == iter->planet1 || p1marked == iter->planet2)
          zoom = bigzoom;
        break;
      case CT_PARTNER:
      case CT_TRANSIT:
        if (p1marked == iter->planet2 || p2marked == iter->planet1)
          zoom = bigzoom;
        break;
      default:
        assert(false);
    ...
    painter->setPen(
        wxPen(color, zoom * config->aspects[iter->aspectType].pen.GetWidth(),
              (wxPenStyle)config->aspects[iter->aspectType].pen.GetStyle()));
    painter->setTextColor(color);
    drawSingleAspect(haspecting->getWesternLongitude(iter->planet1),
                     haspected->getWesternLongitude(iter->planet2),
                     iter->aspectType);
  }
  tryToSetPen(wconf->rPlanets.pen);
  for (iter = al.begin(); iter != al.end(); iter++) {
    paintLengthMarker(aries + haspecting->getWesternLongitude((*iter).planet1),
                      wconf->aspects.radius);
    paintLengthMarker(aries + haspected->getWesternLongitude((*iter).planet2),
                      wconf->aspects.radius);
  }
}
...
void WesternChart::drawHouses() {
  ...
  const int houseNumberStyle =
      chartprops->getWesternChartDisplayConfig().houseNumberStyle;
  bool paintHousenumbers = wconf->houses.showLabels && houseNumberStyle;
  if (paintHousenumbers) {
    if (h2 && !transitsoutside)
      paintHousenumbers = false;
  }
  if (paintHousenumbers) {
    for (int i = HOUSE1; i <= HOUSE12; i++) {
      ...
      houselen = red_deg(h1->getHouse(i == HOUSE12 ? HOUSE1 : i + 1, false) -
                         h1->getHouse(i, false));
      len = red_deg(aries + h1->getHouse(i, false) + .5 * houselen);

      p = frameBorderPointToXy(
          len, GcChartFrame(wconf->houses.radius + wconf->houses.wLabels,
                            wconf->houses.frameType));
      painter->drawTextFormatted(
          MRect(p.real() - psize / 2, p.imag() - psize / 2, psize, psize),
          getHouseNumberFormatted(i, houseNumberStyle), Align::Center);
    }
  }
  ...
  for (int i = HOUSE1; i <= HOUSE12; i++) {
    ...
    paintMarkerLine(aries + len, wconf->houses,
                    GcChartFrame(wconf->houses.radius + wconf->houses.width,
                                 wconf->houses.frameType));
  }
  ...
  if (!transitsoutside) {
    paintArrow(red_deg(aries + h1->getWesternLongitude(OASCENDANT)),
               wconf->angles);
    if (chartprops->getWesternObjectStyle() & OI_ASCENDANT)
      drawAngleSymbol(OASCENDANT);
    if (chartprops->getWesternObjectStyle() & OI_DESCENDANT)
      drawAngleSymbol(ODESCENDANT);

    paintArrow(red_deg(aries + h1->getWesternLongitude(OMERIDIAN)),
               wconf->angles);
    if (chartprops->getWesternObjectStyle() & OI_MERIDIAN)
      drawAngleSymbol(OMERIDIAN);
    if (chartprops->getWesternObjectStyle() & OI_IMUMCOELI)
      drawAngleSymbol(OIMUMCOELI);
  }
}
...
void WesternChart::drawAngleSymbol(const ObjectId &planet) {
  ...
  MPoint point =
      frameBorderPointToXy(len, wconf->angles.magnify(wconf->angles.wLabels));
  // h1rect[planet] = MRect( point.real() - psize/2, point.imag() - psize/2,
  // psize, psize);
  h1rect[planet] =
      MRect(point.real() - psize / 2, point.imag() - psize / 2, psize, psize);

  double zoom = 1.0;
  if (shouldBeMagnified(planet, h1))
  // if (  h1marked == planet &&
  // chartprops->getWesternChartBehaviorConfig().zoomOnMouseOver )
  {
  ...
  painter->drawTextFormatted(h1rect[planet], sp.getPlanetCode(planet),
                             Align::Center);
...
bool WesternChart::shouldBeMagnified(const ObjectId &p, const Horoscope *h) {
  if (!chartprops->getWesternChartBehaviorConfig().zoomOnMouseOver)
    return false;
  ...
  for (iter = al.begin(); iter != al.end(); iter++) {
    switch (charttype) {
    case CT_RADIX:
      if (iter->planet1 == h1marked && iter->planet2 == p)
        return true;
      if (iter->planet2 == h1marked && iter->planet1 == p)
        return true;
      break;
    case CT_PARTNER:
      if (h == h1) {
        if (iter->planet2 == h2marked && iter->planet1 == p)
          return true;
      } else if (h == h2) {
        if (iter->planet1 == h1marked && iter->planet2 == p)
          return true;
    ...
    case CT_TRANSIT:
      if (h == h1) {
        if (iter->planet1 == h2marked && iter->planet2 == p)
          return true;
      } else if (h == h2) {
        if (iter->planet2 == h1marked && iter->planet1 == p)
          return true;
...
void WesternChart::drawAllPlanets(const Horoscope *h, const bool applyFilter) {
  ...
  const bool doSkipAries =
      (config->westernCalculation->ayanamsa == AY_NONE) &&
      (h->getPlanetContext() == PcRadix || h->getPlanetContext() == PcTransit ||
       h->getPlanetContext() == PcDirection);
  ...
  ObjectArray o =
      applyFilter
          ? chartprops->getWesternPlanetList(chartprops->getObjectFilter())
          : obs;
  ...
  if (h == h2) {
    if (transitsoutside)
      frame = wconf->rOuterTransitPlanets;
    else
      frame = wconf->rInnerTransitPlanets;
  } else
    frame = wconf->rPlanets;
  ...
  for (uint i = 0; i < o.size(); i++) {
    ...
    if (planet == OARIES && doSkipAries)
      continue;

    MRect &therect = (h == h2 ? h2rect[planet] : h1rect[planet]);

    // loop for angles in simple radix horoscope
    if (IS_ANGLE_OBJECT(planet) && h2 == (Horoscope *)NULL)
      continue;

    // loop for radix angles in transit horoscope depends on position of transit
    // planets (outer/inner)
    if (IS_ANGLE_OBJECT(planet) && h == h1 && h2 && !transitsoutside)
      continue;
    ...
    // zoom = planet == mp ? .01 * config->westernChartBehavior->zoomFactor
    // : 1.0;
    ...
    if (psymbol == SYMBOL_CODE_ERROR) {
      ...
      align = Align::Center;
      // painter->drawTextFormatted( therect, writer->getObjectName( planet,
      // TF_LONG ), align ); painter->drawTextFormatted( therect,
      // fmt.getObjectName( planet, TF_LONG ), align ); MString objectname =
      // fmt.getObjectName( planet, TF_LONG );
      MString objectname = fmt.getObjectName(planet, TF_MEDIUM);
      // painter->drawMString( therect, fmt.getObjectName( planet, TF_LONG ),
      // align );
    ...
    // Retrogression
    if ((chartprops->getWesternChartDisplayConfig().showRetro) &&
        h->isRetrograde(planet)) {
      setSymbolFontZoom(zoom);

      align = Align::Right + Align::Bottom;
      painter->drawTextFormatted(
          MRect(point.real(), point.imag() + psize / 2, psize, psize),
          // wxT( "_" ),
          SymbolProvider().getRetroCode(MD_RETROGRADE), align);
    }
  }
  setGraphicFont();
}
...
void WesternChart::drawInlayImage() {
  ...
  if (wconf->image.show && !wconf->image.filename.IsEmpty()) {
    ...
    wxImage img =
        ip->getFileBasedBitmap(wconf->image.filename).ConvertToImage();
    ...
    painter->drawBitmap(wxBitmap(img), xcenter - .5 * xnew,
                        ycenter - .5 * height, 0);
  }
}
...
void WesternChart::drawNakshatras() {
  ...
  Lang lang;
  WesternChartConfig *wconf = getWChartConfig();
  GenericNakshatraDasaExpert *expert =
      (GenericNakshatraDasaExpert *)DasaExpertFactory().getDasaExpert(
          D_VIMSOTTARI);
  ...
  */
  ...
  if (wconf->nakshatras.show) {
    ...
    for (i = 0; i < 27; i++) {
      ...
      paintMarkerLine(a, wconf->nakshatras,
                      wconf->nakshatras.magnify(wconf->nakshatras.width));
      a += .5 * NAKSHATRA_LEN;

      pos = frameBorderPointToXy(
          a, wconf->nakshatras.magnify(wconf->nakshatras.wLabels));
      s = lang.getNakshatra27Name((NakshatraId_27)i, TF_MEDIUM);
      painter->drawRotatedText(s, pos.real(), pos.imag(), a - 90);

      pos = frameBorderPointToXy(
          a, wconf->nakshatras.magnify(.5 * wconf->nakshatras.wLabels));
      ...
      s.Clear();
      s << expert->getDasaLordName(expert->getDasaLordByIndex(dasaindex),
                                   TF_LONG)
        << SPACE << years;
      painter->drawRotatedText(s, pos.real(), pos.imag(), a - 90);
...
void WesternChart::drawEastIndianChartCenter() {
  ...
  if (wconf->eastIndianCenter.show) {
    ...
    painter->drawLine(frameBorderPointToXy(aries, frame),
                      frameBorderPointToXy(aries + 210, frame));
    painter->drawLine(frameBorderPointToXy(aries + 30, frame),
                      frameBorderPointToXy(aries + 180, frame));
    painter->drawLine(frameBorderPointToXy(aries + 90, frame),
                      frameBorderPointToXy(aries + 300, frame));
    painter->drawLine(frameBorderPointToXy(aries + 120, frame),
                      frameBorderPointToXy(aries + 270, frame));

    painter->drawLine(MPoint(aries0.real(), cancer0.imag()),
                      frameBorderPointToXy(aries + 330, frame));
    painter->drawLine(MPoint(libra0.real(), cancer0.imag()),
                      frameBorderPointToXy(aries + 60, frame));
    painter->drawLine(MPoint(libra0.real(), capricorn0.imag()),
                      frameBorderPointToXy(aries + 150, frame));
    painter->drawLine(MPoint(aries0.real(), capricorn0.imag()),
...
void WesternChart::drawSigns() {
  ...
  painter->setPen(defaultPen);
  // bool showColors =
  // chartprops->getWesternChartDisplayConfig().showSignColors; if ( !
  // showColors ) setDefaultTextColor();

  // printf( "signs->symbolZoomFactor %d\n", signs->symbolZoomFactor );
  if (signs.labelStyle == 0)
    setSymbolFontZoom(.01 * signs.labelSize);
  else
    setGraphicFontZoom(.01 * signs.labelSize);
  // if ( signs->symbolType != 2 ) painter->setSymbolFont( .01 *
  // signs->symbolZoomFactor ); printf( "WesternChart::paintZodiac zoom factor
  // %d\n", signs->symbolZoomFactor );
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (wconf->textColor.IsOk()) {
      ...
      painter->setTextColor(signs.globalTextColor.IsOk()
                                ? signs.globalTextColor
                                : config->chartColors->getSignFgColor(i));
    } else
      painter->setTextColor(defaultPen.GetColour());
    ...
    if ( chartprops->getWesternChartDisplayConfig().showSignColors )
    {
            ...
            {
                    painter->setTextColor( signs.globalTextColor.IsOk() ?
    signs.globalTextColor : config->chartColors->getSignFgColor( i ));
            }
    }
    */
    ...
    painter->drawRotatedText(s, signpos.real(), signpos.imag(),
                             signs.rotateLabels ? a - 90 : 0);
    a += 30.0;
  }

  if (signs.showInnerFrame)
    paintChartFrame(signs);

  // draw markers between signs if either inner or outer frame is shown, i.e. do
  // not draw if both are hidden
  if (signs.showInnerFrame || signs.showOuterFrame) {
    for (int i = 0; i < 12; i++)
      paintMarkerLine(aries + i * 30, signs, signs.magnify(signs.width));
  }
  if (signs.showOuterFrame)
    paintChartFrame(signs.magnify(signs.width));
}
...
**
******************************************************/
GraphicalChart *ChartFactory::createWesternChart(const ChartType &charttype,
