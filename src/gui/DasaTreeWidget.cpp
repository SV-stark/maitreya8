...
const wxEventType DASATREE_CHANGED = wxID_HIGHEST + 1630;
...
**
******************************************************/
DasaTreeWidget::DasaTreeWidget(wxWindow *parent, ChartProperties *props,
                               wxWindowID winid, Horoscope *horoscope)
    : BasicWidget(parent, props, winid), horoscope(horoscope) {
  uint i;
  ...
  for (int d = 0; d < MAX_DASAEXPERTS; d++)
    experts[d] = factory.getDasaExpert((DasaId)d, horoscope);

  twidget = new TreeWidget(this, winid, wxDefaultPosition, wxDefaultSize,
  ...
  for (i = 0; i < MAX_DASAEXPERTS; i++) {
    if (i == D_VIMSOTTARI || i == D_YOGINI || i == D_KALACHAKRA)
      root = baseid;
    else if (i >= D_ASHTOTTARI && i <= D_SHATTRIMSATSAMA)
      root = conditionalid;
    else
      root = specialid;

    item = new DasaTreeItemClientData(experts[i], (Dasa *)NULL);
    id = twidget->AppendItem(root, getItemLabel(item), PIC_DASAROOT,
                             PIC_DASAROOT, item);
    if (i == D_VIMSOTTARI)
      vimsotid = id;
  ...
  Connect(wxEVT_COMMAND_TREE_ITEM_COLLAPSING,
          MyTreeEventHandler(DasaTreeWidget::collapse));
  Connect(wxEVT_COMMAND_TREE_ITEM_EXPANDING,
          MyTreeEventHandler(DasaTreeWidget::expand));
  Connect(wxEVT_COMMAND_TREE_SEL_CHANGING,
          MyTreeEventHandler(DasaTreeWidget::OnSelChanging));
  // Connect( wxEVT_MOUSEWHEEL, wxMouseEventHandler(
  // DasaTreeWidget::OnMouseWheelEvent ));
...
void DasaTreeWidget::expand(MyTreeEvent &event) {
  ...
  id = event.GetItem();
  DasaTreeItemClientData *data =
      (DasaTreeItemClientData *)twidget->GetItemData(id);
  ...
  if (twidget->GetChildrenCount(id) > 0)
    return;
...
wxString DasaTreeWidget::getItemLabel(DasaTreeItemClientData *item) {
  ...
  if (item->isRootItem()) {
    ...
    s << formatter->formatDateString(
             ...
             config->vedicCalculation->dasaShowDateMode == 1
                 ? item->getDasa()->getEndJD()
                 : Max(jd, item->getDasa()->getStartJD()))
...
void DasaTreeWidget::updateDasa(const wxTreeItemId &masterid) {
  ...
  if (twidget->GetChildrenCount(masterid) == 0)
    return;
